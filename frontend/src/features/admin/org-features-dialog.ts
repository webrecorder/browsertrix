...
export class OrgFeatureFlags extends BtrixElement {
  ...
  flags = new Task(this, {
    task: async () => {
      return featureFlagsMetadataSchema.parse(
        await this.api.fetch<FeatureFlagMetadata[]>("/orgs/feature-flags"),
      );
    },
    autoRun: false,
  });
  ...
  render() {
    ...
    >
      ${this.flags.value
          ...
          ? this.flags.value.map((flag) => {
              ...
              >
                ...
                <span slot="help-text"
                  ...
                  <br />
                  ${msg(
                    html`Enabled for ${organization_count}
                    ${organization_plural}.`,
                  )}
                </span>
              </sl-switch>`;
        ...
        : html`<div class="my-4 text-center">
            ${msg("Failed to load feature flags")}
          </div>`}
    </btrix-dialog>`;
  }
  ...
  async setFlag(name: string, value: boolean) {
    try {
      await this.api.fetch(`/orgs/${this.activeOrg!.id}/feature-flags`, {
        method: "POST",
